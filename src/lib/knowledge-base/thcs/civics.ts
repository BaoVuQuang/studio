// src/lib/knowledge-base/thcs/civics.ts
export const civics = `
Kiến thức Giáo dục công dân Trung học cơ sở

**Các phẩm chất đạo đức:**
- Sống giản dị, trung thực, tự trọng, yêu thương con người.
- Siêng năng, kiên trì, tôn sư trọng đạo, đoàn kết, tương trợ.
- Khoan dung, tự lập, tự chủ, chí công vô tư.
- Năng động, sáng tạo, làm việc có năng suất, chất lượng, hiệu quả.
- Lý tưởng sống của thanh niên.

**Kiến thức pháp luật:**
- **Quyền và nghĩa vụ cơ bản:**
  + Quyền được pháp luật bảo hộ về tính mạng, thân thể, sức khỏe, danh dự, nhân phẩm.
  + Quyền bất khả xâm phạm về chỗ ở.
  + Quyền được bảo đảm an toàn và bí mật thư tín, điện thoại, điện tín.
  + Quyền và nghĩa vụ học tập.
  + Quyền trẻ em.
  + Nghĩa vụ bảo vệ Tổ quốc, nghĩa vụ quân sự.
- **Nhà nước và pháp luật:**
  + Bộ máy nhà nước Cộng hòa Xã hội Chủ nghĩa Việt Nam.
  + Hiến pháp và pháp luật.
  + Các loại vi phạm pháp luật và trách nhiệm pháp lí.
  + Quyền tham gia quản lí nhà nước, quản lí xã hội của công dân.
`;
