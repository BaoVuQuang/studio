...
export default function StudyBuddyClient() {
  ...
  useEffect(() => {
    // Persist the active learning session so reloads keep chat history and document context intact.
    if (typeof window === 'undefined') return;
  ...
  const handleContinueProgress = useCallback(() => {
    // Allow learners to resume a saved session, restoring chat state and context in one click.
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setSelectedSubject(payload.name);
      setChatMessages([]);
      setStep('chat');
      toast({
    ...
    <div className="h-full w-full flex flex-col items-center justify-center p-4">
        {isSelectionStep && savedProgress && (
          <Alert className="mb-6 max-w-2xl">
            ...
            <AlertDescription>
              Bạn có một phiên học gần đây với {savedProgress.selectedSubject ?? 'StudyBuddy AI'}. Bấm tiếp tục để khôi phục lịch sử trò chuyện.
            </AlertDescription>
